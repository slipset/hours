...
(defn to-date [date-or-this]
  (if (= date-or-this ":this")
    (time/trunc-hours (t/now))
    (f/parse (f/formatters :basic-date) date-or-this)))
...
(defn decorate [[start end] client-id report] {:report report
                                               ...
                                               :period (time/all-days start end)
...
(defn get-period-report [db-spec user-id client-id period]
  (let [report (if (= client-id ":all")
    ...
    (->> report
         ...
         (map (fn [[k v]] [k (daily-totals period v)]))
         (into {})
         (decorate period client-id))))

(defn get-weekly-report [db-spec user-id client-id date]
  (get-period-report db-spec user-id client-id (time/week-period (time/prev-monday (to-date date)))))

(defn get-monthly-report [db-spec user-id client-id date]
  (get-period-report db-spec user-id client-id (time/month-period (time/trunc-days (to-date date)))))
