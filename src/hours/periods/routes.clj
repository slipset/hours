...
(defroutes period-routes
  (GET "/:id" [db user-id id] (response (layout/display-edit-period (first (period/by-id {:user_id user-id :id id} db))
                                                                     (prjct/user-projects {:user_id user-id } db))))
  (POST "/:id" [db user-id id date start end project-id description]
        (edit-period! db user-id id date start end description project-id))
